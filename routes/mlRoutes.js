 ...
 * Add to server.js: const mlRoutes = require('./routes/mlRoutes');
 *                   app.use('/ml', mlRoutes);
 */
...
// ── Auth middleware (reuse your existing session check) ───────
...
// ── GET /ml/demand-forecast?commodity=all ──────────────────────
...
// ── GET /ml/stock-depletion?days=30 ───────────────────────────
...
// ── GET /ml/fraud-detect ───────────────────────────────────────
router.get('/fraud-detect', requireAdmin, async (req, res) => {
  try {
...
router.get('/duplicate-detect', requireAdmin, async (req, res) => {
  try {
